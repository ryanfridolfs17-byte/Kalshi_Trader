...
- **Model divergence**: YES >8F = skip, NO >5F = skip (tightened from 10F based on Apr 2026 trade-level data — 14% of NWS-native predictions had 5F+ spread, the noisiest tail). <2F = 1.2x boost.
...
- **Phase 2 trade-level data fixes (April 16, 2026)**: Based on synced trade-level data (500 forecast snapshots, 38 resolved S1, 11 resolved S3). (1) **DAL settlement lock bug fix** — `settlement_lock.evaluate_market_snapshot()` now enforces `SETTLEMENT_LOCK_MIN_LOCAL_HOUR` gate. Previously only `build_signal()` had it, so detection fired at midnight on stale obs and persisted as candidates. Apr 14 DAL 80c -$80 loss traced to this. (2) **Pre-dawn observation guard** — `trade_intelligence._get_metar_todays_high()` and `_fetch_nws_todays_high()` now require at least one observation from after `OBSERVATION_MIN_LOCAL_HOUR` (default 6 AM local) before returning a value. Defense-in-depth at the observation layer; suppresses false same-day highs from overnight cache hits. New `[METAR-PREDAWN]` diag log. (3) **April/May seasonal multiplier removed** (Codex commit `eafc975`) — backtest MAE for both months equals overall avg (2.24F vs 2.26F), no penalty justified. (4) **October seasonal multiplier added: 0.87x** — backtest shows October is the structurally easiest month (1.96F MAE vs 2.26F avg). Bot will take more trades in best-forecasting month. (5) **MAX_MODEL_DIVERGENCE_NO_F: 10 → 5** — 14% of NWS-native predictions had 5F+ spread (the noisiest tail). Tightening cuts that without much volume loss. (6) **Verdict-level scorecard instrumentation** (Codex commit `eafc975`) — new `verdict_stats`, `strong_resolved/wins/losses`, `confirm_resolved/wins/losses` fields on every strategy scorecard. First data shows STRONG (36 resolved, 53% WR, +$251) outperforms CONFIRM (2 resolved, 0/2, -$140) under NWS native — opposite of the historical Open-Meteo pattern. Block remains for now; instrumentation captures data for future conditional unblock.
