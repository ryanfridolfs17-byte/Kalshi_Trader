...
- **`signal_confirmer.py`** — Weather voting system from 5 independent sources: 4 deterministic models (GFS/HRRR, ECMWF, ICON, GEM) via Open-Meteo + NWS station point forecast (api.weather.gov). Outputs: STRONG (3+ agree, 1.5x), CONFIRM (2, 1.0x), WEAK (0.5x), REJECT. NWS source uses tighter 2°F abstain zone (vs 3°F for models) since it's most authoritative for settlement.
